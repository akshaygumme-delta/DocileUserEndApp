...
def test_address():
    ...
    try:
        element_finder.find_clickable_element_by_xpath(
            '//android.widget.TextView[@text="Enter complete address"]').click()
        xpath = ['//android.widget.EditText[@text="House No. & Floor *"]',
                 '//android.widget.EditText[@text="Building & Block No *"]',
                 '//android.widget.EditText[@text="Landmark(Optional)"]']
        send = ['203 delta iot', 'second floor', '']

        for i in range(len(xpath)):
            element_finder.find_send_data_by_xpath(xpath[i]).send_keys(send[i])
        element_finder.find_clickable_element_by_xpath('//android.widget.EditText[@text="Landmark(Optional)"]').click()
        screenshot_path = "Address.png"
        driver.save_screenshot(screenshot_path)
        screenshot = xlImage(screenshot_path)
        assert True
        logging.info("ADDRESS SAVED SUCCESSFULLY")

        write_test_result(issue_id='6', issue_description='Adding Address',
                          test_result='Passed', screenshot=screenshot,
                          severity_level='None', steps_to_reproduce='None',
                          expected_behavior='ADDING ADDRESS',
                          actual_behavior='As expected',
                          device_platform='Android', additional_notes_comments='No issues')

    except Exception as e:
        logging.error("address failed to add")
        screenshot_path = "Address_fail.png"
        driver.save_screenshot(screenshot_path)
        screenshot = xlImage(screenshot_path)
        write_test_result(issue_id='6', issue_description='Adding Address',
                          test_result='Fail', screenshot=screenshot,
                          severity_level='High', steps_to_reproduce='need to check manually first and check elements '
                                                                     'for',
                          expected_behavior='ADDING ADDRESS',
                          actual_behavior='Not as expected',
                          device_platform='Android', additional_notes_comments='No issues')
